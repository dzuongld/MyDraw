...
Figure * ControlPoint::ofFigure() {
	return this->figure;
}

ControlPoint* ControlPoint::closerTo(ControlPoint* c1, ControlPoint* c2) {
	int x0 = this->getX();
	int y0 = this->getY();
	int x1 = c1->getX();
	int y1 = c1->getY();
	...
	int d1 = (x1 - x0)*(x1 - x0) + (y1 - y0)*(y1 - y0);
	int d2 = (x2 - x0)*(x2 - x0) + (y2 - y0)*(y2 - y0);
